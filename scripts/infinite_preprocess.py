...
from utils import MinioClient, PaperspaceClient
...
if __name__ == "__main__":
    ...
    pClient = PaperspaceClient(os.environ["PAPERSPACE_API_KEY"])
    ...
    while True:
        try:
            ...
            files_to_process = []
            files = s3_client.get_list_of_files(bucket_name)
            for f in files:
                filename = f[: f.find(".")]
                if (
                    complete_suffix not in f
                    and f"{filename}-{complete_suffix}.tar.gz" not in files
                ):
                    files_to_process.append(f)

            if len(files_to_process) > 0:
                logging.info("Pending job found, start processing")
                notebooks = pClient.get_notobooks_by_project_id(project_id)
                running = False
                for notebook_status in notebooks:
                    if notebook_status.state == "Running":
                        ...
                        pClient.notebooks_client.delete(id=notebook_status.id)
                if running:
                    ...
                    notebook_end_time = notebook_start_time + datetime.timedelta(
                        hours=notebook_status.shutdown_timeout
                    )
                    notebook_url = notebook_base_url.format(
                        notebook_status.id,
                        notebook_status.cluster_id,
                        notebook_status.token,
                    )
                    ...
                    try:
                        element = WebDriverWait(driver, 20).until(
                            EC.element_to_be_clickable(
                                (
                                    By.XPATH,
                                    '//div[@title="Start a new terminal session"]',
                                )
                            )
                        )
                        element.click()
                    ...
                    while True:
                        try:
                            ...
                            logging.exception("Failed to enter command into terminal")
                            break
                    continue
                else:
                    gpu = pClient.find_available_gpu(priority)
                    if gpu is None:
                        while True:
                    ...
                    try:
                        ...
                        environment.update(
                            {e.replace("ENV_", ""): os.environ[e] for e in extra_env}
                        )
                        ...
                        notebook_id = pClient.notebooks_client.create(
                            machine_type=gpu,
                            container="paperspace/gradient-base:pt112-tf29-jax0317-py39-20230125",
                            project_id=project_id,
                            name="auto",
                            command="bash entry.sh >> /tmp/run.log & jupyter lab --allow-root --ip=0.0.0.0 --port=8890 --no-browser --ServerApp.trust_xheaders=True --ServerApp.disable_check_xsrf=False --ServerApp.allow_remote_access=True --ServerApp.allow_origin='*' --ServerApp.allow_credentials=True",
                            shutdown_timeout="6",
