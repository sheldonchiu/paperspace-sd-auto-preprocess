...
def main():
    ...
    uploader = ProcessPoolExecutor(
        max_workers=2
    )  # low chance of process randomly dead, set to 2 to avoid stuck for upload
    ...
    for f in files:
        ...
        if settings.use_result_as_input:
            ...
            if (
                f.endswith(f"-{settings.target_complete_suffix}.tar.gz")
    ...
    results = downloader.map(
        download_with_queue,
        [
            (bucket_name, f, osp.join(settings.data_download_path, osp.basename(f)))
    ...
    if settings.vae_model_url:
        ...
        sd_model_path = download_model(
            osp.join(settings.model_path, "stable-diffusion"), settings.vae_model_url
        )
    else:
    ...
    for file in files_to_process:
        ...
        if next(results) and (target_dir := extract(local_path)):
            try:
                ...
                filter_dst = (
                    f"{target_dir}_filter" if settings.enable_filter else target_dir
                )
                debug_dir = (
                    f"{target_dir}_debug" if settings.save_img_for_debug else None
                )
                ...
                if settings.tag_using_wd14:
                    with jobContext(job_name="tag", file=file):
                        ...
                        wd_args = prepare_wd_parser(
                            target_dir,
                            thresh=settings.wd14_thresh,
                            batch_size=settings.wd14_batch_size,
                            caption_extention=tag_extension,
                            general_thresh=settings.wd14_general_thresh,
                            character_thresh=settings.wd14_character_thresh,
                        )
                        task = context.Process(
                            target=tag_images_by_wd14_tagger.main, args=(wd_args,)
                        )
                        task.start()
                        task.join()
                ...
                if (
                    settings.use_original_tags
                    or settings.tag_using_wd14
                    or settings.enable_caption
                ):
                    with jobContext(job_name="merge", file=file):
                        meta_file = osp.join(filter_dst, "meta_cap_dd.json")
                        if settings.use_original_tags:
                            merge_tag_extension = test_tag_extension(
                                filter_dst, ".txt", tag_extension
                            )
                            if merge_tag_extension is None:
                                logger.error(
                                    "No tag file in source directory, unknow issue"
                                )
                                raise Exception
                        else:
                            merge_tag_extension = tag_extension
                        merge_arg = prepare_merge_parser(
                            filter_dst, meta_file, merge_tag_extension
                        )
                        merge_dd_tags_to_metadata.main(merge_arg)
                        if settings.enable_caption:
                            merge_arg = prepare_merge_parser(
                                filter_dst, meta_file, caption_extension
                            )
                ...
                with jobContext(job_name="bucket", file=file):
                    ...
                    bucket_args = prepare_bucket_parser(
                        filter_dst,
                        meta_file,
                        lat_file,
                        sd_model_path,
                ...
                uploader.submit(
                    compress_and_upload, folders_to_compress, output_path, bucket_name
                )
