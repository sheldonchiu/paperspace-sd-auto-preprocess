...
def prepare_wd_parser(
    train_data_dir: str,
    thresh: float = 0.35,
    batch_size: int = 4,
    caption_extention: str = ".txt",
    ...
    args = parser.parse_args(
        [
            ...
            str(thresh),
            "--general_threshold",
            str(general_thresh) if general_thresh else str(thresh),
            "--character_threshold",
            str(character_thresh) if character_thresh else str(thresh),
            "--batch_size",
            str(batch_size),
            "--max_data_loader_n_workers",
            "0",
...
def prepare_caption_parser(
    train_data_dir: str, batch_size: int = 4, caption_extention: str = ".caption"
) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "train_data_dir", type=str, help="directory for train images / 学習画像データのディレクトリ"
    ...
    parser.add_argument(
        "--caption_weights",
        type=str,
        default="https://huggingface.co/sheldonxxxx/ofa_for_repo/resolve/main/caption_huge_best.pt",
    ...
    parser.add_argument(
        "--batch_size", type=int, default=3, help="batch size in inference / 推論時のバッチサイズ"
    )
    parser.add_argument(
        "--max_data_loader_n_workers",
        type=int,
    ...
    parser.add_argument(
        "--max_length", type=int, default=16, help="max length of caption / captionの最大長"
    )
    parser.add_argument(
...
def prepare_bucket_parser(
    ...
    parser.add_argument("--upscale", action="store_true", help="upscale before resize")
    parser.add_argument(
        "--upscale_enable_reso",
        type=int,
        default=1000 * 1000,
    ...
    parser.add_argument("--upscale_outscale", type=int, default=2, help="")
    parser.add_argument(
    ...
    parser.add_argument("--upscale_tile_pad", type=int, default=10, help="Tile padding")
    parser.add_argument(
        "--upscale_pre_pad", type=int, default=0, help="Pre padding size at each border"
    )
    s = [
        train_data_dir,
        in_json,
...
def check_work_queue():
    ...
    for f in files:
        filename = f[: f.find(".")]
        if (
            settings.complete_suffix not in f
            and f"{filename}-{settings.complete_suffix}.tar.gz" not in files
        ):
...
def download_with_queue(data):
    ...
    while True:
        ...
        if queue_size > settings.prefetch_num_file:
            logger.debug(
                f"Current number of files in queue is {queue_size}, will go to sleep"
            )
            time.sleep(120)
        else:
            break
...
def setup_logger(logger):
    ...
    logging.getLogger("discord_webhook").setLevel(
        logging.FATAL
    )  # discord_webhook.webhook - ERROR - Webhook rate limited: sleeping for 0.235 seconds...
    ...
    discord_handler = DiscordHandler(
        "Paperspace preprocess", settings.discord_webhook_url
    )
...
def cache_progress_watcher(
    bucket_name, target_dir, key, file_extension, recursive=False, interval=60 * 5
):
    ...
    while not osp.isfile(complete_mark):
        files = [
            ...
            for f in glob(
                osp.join(
                    target_dir,
                    f"**/*{file_extension}" if recursive else f"*{file_extension}",
                ),
                recursive=recursive,
            )
            if f not in record
...
def save_setting_to_file(file_path):
    variables = {
        name: value
